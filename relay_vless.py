...
MAX_GLOBAL_CONNECTIONS = int(os.environ.get("MAX_GLOBAL_CONNECTIONS", "200"))
MAX_UUID_CONNECTIONS = int(os.environ.get("MAX_UUID_CONNECTIONS", "5"))
MAX_IP_CONNECTIONS = int(os.environ.get("MAX_IP_CONNECTIONS", "3"))

def _connection_count(uuid=None, ip=None):
    count = 0
    for c in connections.values():
        if uuid and c.get("uuid") == uuid:
            count += 1
        if ip and c.get("ip") == ip:
            count += 1
    return count
...
async def websocket_tunnel(ws: WebSocket, uuid: str):
    # جلوگیری از مصرف بیش از حد منابع توسط کانکشن‌های زیاد
    ip = _ws_client_ip(ws)
    if len(connections) >= MAX_GLOBAL_CONNECTIONS:
        await ws.close(code=1013, reason="server busy")
        return
    if _connection_count(uuid=uuid) >= MAX_UUID_CONNECTIONS:
        await ws.close(code=1008, reason="connection limit")
        return
    if _connection_count(ip=ip) >= MAX_IP_CONNECTIONS:
        await ws.close(code=1008, reason="ip connection limit")
        return
